...
def execute(conn, inputs, params, outputs, reportFileName):
    ...
    构造模型
    '''
    model = KNeighborsClassifier(n_neighbors = int(params['n_neighbors']),weights =params['weights'],algorithm =params['algorithm'],leaf_size=int(params['leaf_size']))
    ...
    report.h1('KNN算法')
    model_params={}
    model_params['K值']=params['n_neighbors']
    model_params['投票权重类型']=params['weights']
    model_params['计算最近邻的算法']=params['algorithm']
    ...
    model_params['模型有效指标']=model.effective_metric_
    a=pd.DataFrame([model_params.keys(),model_params.values()]).T
    a.columns=['参数名称','参数值']
    report.h3('模型属性')
    report.p("输出模型的属性信息。")
