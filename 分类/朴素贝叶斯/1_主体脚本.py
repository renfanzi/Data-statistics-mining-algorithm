...
def execute(conn, inputs, params, outputs, reportFileName):
	...
	if params['method'] == 'gaussianNb':
	    ...
	    report.table(a)
	    model_params={}
	    model_params['类别数']=model.classes_
	    #np.around(acc,decimals=2)
	    model_params['sigma_']=np.around(model.sigma_,decimals=4)
	    ...
	    model_params={}
	    model_params['类别数']=model.classes_
	    model_params['coef_']=np.around(model.coef_,decimals=4)
	    model_params['feature_log_prob_']=np.around(model.feature_log_prob_,decimals=4)
	    a=pd.DataFrame([model_params.keys(),model_params.values()]).T
	    a.columns=['参数名称','参数值']
	    report.h3('模型属性')
	    report.p("输出模型的属性信息。")
	    ...
	    model_params['coef_']=np.around(model.coef_,decimals=4)
	    model_params['feature_log_prob_']=np.around(model.feature_log_prob_,decimals=4)
	    model_params['intercept_']=np.around(model.intercept_,decimals=4)
	    model_params['feature_count_']=np.around(model.feature_count_,decimals=4)
	    a=pd.DataFrame([model_params.keys(),model_params.values()]).T
	    a.columns=['参数名称','参数值']
	    report.h3('模型属性')
	    report.p("输出模型的属性信息。")
