...
def evaluate(conn, model, inputs, params, outputs, reportFileName):
	...
	plt.title('Prediction of LASSO Regression Model')
	...
	plt.legend()
	plt.xlabel('index')
	plt.ylabel(y_test.columns.values[0])
	plt.tight_layout()
	plt.savefig('pre_view.png')
	report.image('pre_view.png')
	data_out=pd.concat([X_test,y_test,pd.DataFrame({params['add_col']:y_pred})],axis=1)
